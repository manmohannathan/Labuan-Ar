// Import loading engine from the libs folder relatively
...
// Instantiating standard HTML5 Audio components using relative tracking
...
const initializeMindAR = () => {
  return new window.MINDAR.IMAGE.MindARThree({
    ...
    imageTargetSrc: '../assets/targets/testBuilding.mind', // Relative path to tracking file
  });
};
...
const loadBuildingModel = async () => {
  const building = await loadGLTF('../assets/models/testBuilding/testBuilding.gltf'); // Relative path to GLTF
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('narrator-btn').addEventListener('click', () => {
    ...
    if (!isAudioPlaying) {
      activeAudio.play().catch(err => console.warn("Audio context restricted by mobile system settings:", err));
      isAudioPlaying = true;
  ...
  const start = async () => {
    ...
    // Remove background solid color so camera video track shows safely through
    ...
    // Fire camera stream permissions prompt asynchronously
